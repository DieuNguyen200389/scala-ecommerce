...
@Singleton
class ExternalProductClient @Inject()(ws: WSClient, config: Configuration)(implicit ec: ExecutionContext)
  extends AbstractHttpClient(ws) with Logging {

  def baseUrl: String = config.get[String]("external.products.auth.basic.url")

  def username: String = config.get[String]("external.products.auth.basic.username")

  def password: String = config.get[String]("external.products.auth.basic.password")
